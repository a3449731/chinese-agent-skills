...
# Skill root: first CLI arg if given, else two levels up from this file (scripts/ -> skill root).
# NOTE: the audit asserts against the ENGLISH originals (SKILL.md, references/*.md),
# so run it against the docs tree, not a translated install copy.
SK = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else pathlib.Path(__file__).resolve().parent.parent
